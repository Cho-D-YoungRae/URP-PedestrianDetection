...
class SDSMultiBoxLoss(nn.Module):
    ...
    def __init__(self, priors_cxcy, usages_seg_feats, 
        ...
        self.seg_sizes = []
        for i, seg_size in enumerate([38, 19, 10, 5, 3, 1]):
            if usages_seg_feats[i]:
                self.seg_sizes.append(seg_size)
        

    def forward(self, predicted_locs, predicted_scores, boxes, labels, segmentations, origin_seg_labels):
        """
        Forward propagation.
        ...
        for i, seg_size in enumerate(self.seg_sizes):
            ...
            seg_label = F.interpolate(origin_seg_labels, size=size)
            seg = segmentations[i]
            seg_loss = self.cross_entropy(seg.view(-1, n_classes), seg_label.view(-1))
